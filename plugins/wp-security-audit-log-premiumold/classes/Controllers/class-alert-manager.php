	...
	class Alert_Manager {
		...
		public static function log( $event_id, $event_data = array() ) {
			...
			foreach ( self::get_loggers() as $logger ) {
				...
				/* @premium:start */
				// phpcs:enable
				if ( ! ( $logger instanceof \WSAL\Loggers\Database_Logger ) && ! ( $logger instanceof \WSAL\Loggers\Notification_Logger ) ) {
					if ( $logger instanceof \WSAL\Loggers\WSAL_Ext_MirrorLogger ) {
						$logger->log( $event_id, $event_data );

						if ( \method_exists( $logger, 'should_be_scheduled' ) && $logger->should_be_scheduled() ) {
							$external_logger = true;
						}
					} elseif ( $logger->is_external() ) {
						$external_logger = true;
				...
				if ( $logger instanceof \WSAL\Loggers\Database_Logger ) {
					if ( ! Connection::is_writing_directly() ) {
						$external_logger = true;
						// Skip database logger if connected to external database.
						continue;
					}
			...
			if ( $external_logger || $db_enabled_logger ) {
				...
				try {
					...
					if ( function_exists( '\as_enqueue_async_action' ) ) {
						...
						Logger::log( 'Essential function is missing - check you Action Scheduler install!' );
						Logger::log( 'Unable to write data: Event ID: ' . $event_id . ' Data: ' . print_r( $event_data, true ) );
					}
		...
		public static function log_scheduled_event( $job_data ) {
			foreach ( self::get_loggers() as $logger ) {
				if ( $logger instanceof \WSAL\Loggers\Database_Logger ) {
					if ( ! Connection::is_external() || Connection::is_writing_directly() ) {
