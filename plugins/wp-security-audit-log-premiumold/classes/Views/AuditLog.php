...
class WSAL_Views_AuditLog extends WSAL_AbstractView {
	...
	public function admin_notices() {
		...
		if ( $is_current_view ) {
			...
			if ( class_exists( '\WSAL\Extensions\ExternalDB\Mirrors\WSAL_Ext_Mirrors_AWSCloudWatchConnection' ) ) {
				\WSAL\Extensions\ExternalDB\Mirrors\WSAL_Ext_Mirrors_AWSCloudWatchConnection::display_no_aws_sdk_notice();
			}
			if ( class_exists( 'WSAL_Extension_Manager' ) ) {
				WSAL_Extension_Manager::display_helper_plugin_needed_notice();
			}
		}
		// phpcs:disable
		/* @premium:end */
		...
		if ( in_array( $screen->base, array( 'toplevel_page_wsal-auditlog', 'wp-activity-log_page_wsal-usersessions-views', 'wp-activity-log_page_wsal-np-notifications', 'wp-activity-log_page_wsal-rep-views-main' ), true ) ) {
			$is_disabled = Settings_Helper::is_database_logging_enabled();
			if ( ! $is_disabled ) {
	...
	public function ajax_switch_db() {
		...
		if ( isset( $post_array['selected_db'] ) ) {
			WP_Helper::set_transient( 'wsal_wp_selected_db', \sanitize_text_field( \wp_unslash( $post_array['selected_db'] ) ), HOUR_IN_SECONDS );
	...
	public function dismiss_missing_aws_sdk_nudge() {
		...
		if ( ! array_key_exists( 'nonce', $_POST ) || ! wp_verify_nonce( sanitize_text_field( \wp_unslash( $_POST['nonce'] ) ), 'dismiss_missing_aws_sdk_nudge' ) ) {
			wp_send_json_error();
		}

		Settings_Helper::delete_option_value( 'show-aws-sdk-config-nudge-4_3_2' );
		wp_send_json_success();
	}
	...
	public function dismiss_helper_plugin_needed_nudge() {
		...
		if ( ! array_key_exists( 'nonce', $_POST ) || ! wp_verify_nonce( \sanitize_text_field( \wp_unslash( $_POST['nonce'] ) ), 'dismiss_helper_plugin_needed_nudge' ) ) {
			wp_send_json_error();
		}

		Settings_Helper::delete_option_value( 'show-helper-plugin-needed-nudge' );
		wp_send_json_success();
	}
	// phpcs:disable
	/* @premium:end */
