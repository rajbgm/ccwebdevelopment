	...
	class Settings_Builder {
		...
		private static function builder() {
			if ( \class_exists( '\WSAL\Extensions\Helpers\Notification_Helper' ) ) {
				?>
				<div class="wsal-custom-builder-selector">
					<div id="builder-basic"></div>
						...
						if ( isset( $decoded_data['sql'] ) ) {

							$data     = $decoded_data['sql'];
							$pure_sql = true;
						}
						?>
				<!-- <input type="button" id="btn-get-sql" class="btn btn-success set-sql" value="Set rules from SQL"/> -->
				</div>
				<script>

					<?php
					if ( ! empty( $data ) && ! $pure_sql ) {
						?>
					var rules_basic = <?php echo $data; ?>;
						<?php
					}
					?>

					// Fix for Selectize
					jQuery('#builder-basic').on('afterCreateRuleInput.queryBuilder', function(e, rule) {
					...
					// Fix for rules count
					// jQuery('#builder-basic').on('beforeAddRule.queryBuilder', function(e, rule) {

					// 	var count = (function loop(rules, counter=0) {

					// 		if (null === rules) return 0;

					// 		jQuery.each(rules, function(ruleOrGroup, rulesArray) {
					// 			// this item is a group: recurse
					// 			if ('rules' == ruleOrGroup) {
					// 				counter += loop(rulesArray); // recurse
					// 			}
					// 			// this item is a rule
					// 			counter++;
					// 			// else if (ruleOrGroup.field == 'myIntegerField' && ruleOrGroup.value > 3) {
					// 			// 	rulesOverThree.push(ruleOrGroup);
					// 			// }
					// 		});

					// 		return counter;

					// 	}(jQuery('#builder-basic').queryBuilder('getRules')));

					// 	console.log( count );
					// });

					// Fix for Bootstrap Datepicker
					jQuery('#builder-basic').on('afterUpdateRuleValue.queryBuilder', function(e, rule) {
					...
					jQuery(document).ready(function() {
						...
						filters: [{
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['username']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['username']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['username']['type']; ?>',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['username']['operators']; ?>],
							validation: {
								format: /^[A-Za-z0-9\_\.\ \-\@]{3,}$/,
							},
							},{
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['postid']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['postid']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['postid']['type']; ?>',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['postid']['operators']; ?>],
							
							},
							
							{
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['orderid']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['orderid']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['orderid']['type']; ?>',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['orderid']['operators']; ?>],
							
							},
														
							{
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['productid']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['productid']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['productid']['type']; ?>',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['productid']['operators']; ?>],
							
							},
														
							{
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['couponid']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['couponid']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['couponid']['type']; ?>',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['couponid']['operators']; ?>],
							
							},
														
							{
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['productstatus']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['productstatus']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['productstatus']['type']; ?>',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['productstatus']['operators']; ?>],
							plugin: 'selectize',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['poststatus']['operators']; ?>],
								plugin_config: {
									valueField: 'id',
									labelField: 'name',
									searchField: 'name',
									...
									onInitialize: function() {
										...
										[
											...
											<?php
											$options = WP_Helper::get_woo_product_statuses();
											foreach ( $options as $key => $option ) {
												echo '{' .
												'id' . ':' . '"' . $key . '"' . ',' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
												'name' . ':' . '"' . $option . '"' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
												'},';
											}
											?>
							...
							{
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['sku']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['sku']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['sku']['type']; ?>',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['sku']['operators']; ?>],
							
							},
							
							{
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['clientip']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['clientip']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['clientip']['type']; ?>',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['clientip']['operators']; ?>],
							validation: {
								//format: /^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/,
								format: /^((?:[0-9]{1,3}\.?){1,}[,\s]*){1,}$/,
							},
							},{
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['date']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['date']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['date']['type']; ?>',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['date']['operators']; ?>],
							validation: {
								format: 'YYYY-MM-DD'
							},
							plugin: 'datepicker',
							...
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['time']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['time']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['time']['type']; ?>',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['time']['operators']; ?>],
							input: function(rule, name) {
								return '\
									<input name="'+ name +'_1" type="time" >';
							},
							...
							valueSetter: function(rule, value) {
								if (rule.operator.nb_inputs > 0) {
									...
									rule.$el.find('.rule-value-container [name$=_1]').val(value).trigger('change');
								}
							}
						},{
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['event_id']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['event_id']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['event_id']['type']; ?>',
							plugin: 'selectize',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['event_id']['operators']; ?>],
							plugin_config: {
								valueField: 'id',
								labelField: 'name',
								searchField: 'name',
								...
								onInitialize: function() {
									...
									[
										...
										<?php
										foreach ( Alert_Manager::get_alerts() as $alert ) {
											echo '{' .
												'id' . ':' . $alert['code'] . ',' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
												'name' . ':' . '"' . $alert['code'] . '"' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
											'},';
							...
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['object']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['object']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['object']['type']; ?>',
							plugin: 'selectize',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['object']['operators']; ?>],
							plugin_config: {
								valueField: 'id',
								labelField: 'name',
								searchField: 'name',
								...
								onInitialize: function() {
									...
									[
										...
										<?php
										foreach ( Alert_Manager::get_event_objects_data() as $key => $role ) {
											echo '{' .
												'id' . ':' . '"' . $key . '"' . ',' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
												'name' . ':' . '"' . $role . '"' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
											'},';
							...
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['eventtype']['interpolate']; ?>',
							label:  '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['eventtype']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['eventtype']['type']; ?>',
							plugin: 'selectize',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['eventtype']['operators']; ?>],
							plugin_config: {
								valueField: 'id',
								labelField: 'name',
								searchField: 'name',
								...
								onInitialize: function() {
									...
									[
										...
										<?php
										foreach ( Alert_Manager::get_event_type_data() as $key => $role ) {
											echo '{' .
												'id' . ':' . '"' . $key . '"' . ',' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
												'name' . ':' . '"' . $role . '"' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
											'},';
							...
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['severity']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['severity']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['severity']['type']; ?>',
							plugin: 'selectize',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['severity']['operators']; ?>],
							plugin_config: {
								valueField: 'id',
								labelField: 'name',
								searchField: 'name',
								...
								onInitialize: function() {
									...
									[
										...
										if ( class_exists( Constants::class, false ) ) {
											$options = Constants::get_severities_with_codes();
											foreach ( $options as $key => $option ) {
												echo '{' .
												'id' . ':' . '"' . $key . '"' . ',' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
												'name' . ':' . '"' . $option . '"' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
							...
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['roles']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['roles']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['roles']['type']; ?>',
							plugin: 'selectize',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['roles']['operators']; ?>],
							plugin_config: {
								valueField: 'id',
								labelField: 'name',
								searchField: 'name',
								...
								onInitialize: function() {
									...
									[
										...
										foreach ( $options as $key => $option ) {
											echo '{' .
											'id' . ':' . '"' . $key . '"' . ',' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
											'name' . ':' . '"' . translate_user_role( $option ) . '"' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
											'},';
										}
										?>
									].forEach(function(item) {
							...
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['affected_user_role']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['affected_user_role']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['affected_user_role']['type']; ?>',
							plugin: 'selectize',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['affected_user_role']['operators']; ?>],
							plugin_config: {
								valueField: 'id',
								labelField: 'name',
								searchField: 'name',
								...
								onInitialize: function() {
									...
									[
										...
										foreach ( $options as $key => $option ) {
											echo '{' .
											'id' . ':' . '"' . translate_user_role( $option ) . '"' . ',' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
											'name' . ':' . '"' . translate_user_role( $option ) . '"' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
											'},';
										}
										?>
							...
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['posttype']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['posttype']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['posttype']['type']; ?>',
							plugin: 'selectize',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['posttype']['operators']; ?>],
							plugin_config: {
								valueField: 'id',
								labelField: 'name',
								searchField: 'name',
							...
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['custom_field_name']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['custom_field_name']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['custom_field_name']['type']; ?>',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['custom_field_name']['operators']; ?>],
							validation: {
								format: /^[A-Za-z0-9\_\.\ \-\@]{3,}$/,
							},
						},{
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['poststatus']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['poststatus']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['poststatus']['type']; ?>',
							plugin: 'selectize',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['poststatus']['operators']; ?>],
							plugin_config: {
								valueField: 'id',
								...
								onInitialize: function() {
									...
									[
										...
										<?php
										$options = WP_Helper::get_post_statuses();
										foreach ( $options as $key => $option ) {
											echo '{' .
											'id' . ':' . '"' . $key . '"' . ',' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
											'name' . ':' . '"' . $option . '"' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
						...
						if ( WP_Helper::is_multisite() ) {
							...
							?>
						,{
							id: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['siteid']['interpolate']; ?>',
							label: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['siteid']['label']; ?>',
							type: '<?php echo Notification_Helper::get_notifications_fields_name_mapping()['siteid']['type']; ?>',
							plugin: 'selectize',
							operators: [<?php echo Notification_Helper::get_notifications_fields_name_mapping()['siteid']['operators']; ?>],
							plugin_config: {
								valueField: 'id',
								labelField: 'name',
								searchField: 'name',
								...
								onInitialize: function() {
									...
									[
										...
										foreach ( $sites as $site ) {
											echo '{' .
											'id' . ':' . '"' . $site->blog_id . '"' . ',' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
											'name' . ':' . '"' . $site->blogname . '"' . // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
											'},';
										}
										?>
									].forEach(function(item) {
					...
					<?php } ?>],
						...
						if ( $pure_sql ) {
							...
							jQuery('#builder-basic').queryBuilder('setRulesFromSQL', " <?php echo $data; ?> ");
						<?php } ?>
					...
					jQuery('#btn-set').on('click', function() {
						jQuery('#builder-basic').queryBuilder('setRules', rules_widgets);
					});
