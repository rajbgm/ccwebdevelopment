...
if ( ! class_exists( 'WpSecurityAuditLog' ) ) {
	...
	class WpSecurityAuditLog {
		...
		private static function init_hooks() {
			...
			/* @premium:start */
			// phpcs:enable
			\add_filter( 'user_row_actions', array( __CLASS__, 'add_user_events_link' ), 10, 2 );
			// phpcs:disable
			/* @premium:end */
			// phpcs:enable
		}
		 ...
		 * @since 5.0.0
		 */
		public static function add_user_events_link( $actions, $user ) {
		...
		private static function init_freemius() {
			...
			if ( $is_admin_blocking_plugins_support_enabled || is_admin() || WP_Helper::is_login_screen() || self::is_rest_api() || ( defined( 'DOING_CRON' ) && DOING_CRON ) || ( defined( 'WP_CLI' ) && \WP_CLI ) ) {
				...
				if ( wsal_freemius()->can_use_premium_code() || wsal_freemius()->is_plan__premium_only( 'starter' ) ) {
					self::include_extensions__premium_only();
				}
				// phpcs:disable
				/* @premium:end */
				// phpcs:enable
		...
		public static function render_footer() {
			...
			$live_events_enabled = $is_premium && ! \WSAL\Helpers\Settings_Helper::get_boolean_option_value( 'disable-admin-bar-notif', true ) && 'real-time' === WSAL\Helpers\Settings_Helper::get_option_value( 'admin-bar-notif-updates', 'page-refresh' );
			// phpcs:disable
			/* @premium:end */
			// phpcs:enable
